...
export function Livros({ currentUser }) {
  ...
  async function loadReferences() {
    const [authors, categories, publishers, shelves, sections, copies] = await Promise.all([
  ...
  const shelfGroups = useMemo(() => {
    const locatedBookIds = new Set()
    const groups = prateleiras.map((shelf) => {
      const groupBooks = visibleBooks.filter((book) => exemplares.some((copy) => copy.livro_id === book.id && copy.prateleira_id === shelf.id))
      groupBooks.forEach((book) => locatedBookIds.add(book.id))
      return { id: shelf.id, name: `Prateleira ${String(shelf.numero).padStart(2, '0')}`, books: groupBooks }
    })
    const unlocatedBooks = visibleBooks.filter((book) => !locatedBookIds.has(book.id))
    if (unlocatedBooks.length) groups.push({ id: 'unlocated', name: 'Sem prateleira', books: unlocatedBooks })
    return groups.filter((group) => group.books.length)
  }, [exemplares, prateleiras, visibleBooks])
